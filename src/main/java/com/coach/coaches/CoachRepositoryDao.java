...
public class CoachRepositoryDao {
	...
	private static List<Coach> buildCoachesList() {
		...
		coach = Coach.builder().id("0")
				...
				.level("Former French top 20").name("Guillaume Tromp").picture("guillaume.jpg").sport(Badminton)
				.tariff("5€")
				.tariffDescription(
		...
		return Coach.builder().id("4")
				.description(
						"<p>J'ai commencé le squash a l'âge de 11 ans, à St Raphaël, ma mère travaillait dans le club, j'étais sur les courts tous les soirs après l'école ;) \r\n"
								...
								+ "</p>")
				.email("alex.mullex@yahoo.fr").languagesSpoken(Arrays.asList(new Language[] { French })).level("2A")
				.name("Alex Muller").picture("alex_muller.jpg").sport(Squash).tariff("12€")
	...
	private static Coach buildAndrew() {
		return Coach.builder().id("3")
				...
				.email("zerocityhots@gmail.com").languagesSpoken(Arrays.asList(new Language[] { English }))
				.level("Rank 1").name("Andrew (PentaUnleash)").picture("Andrew.png").sport(HeroesOfTheStorm)
				.tariff("3$")
				.tariffDescription(
	...
	public List<User> getAllCoaches(Sport sport) {
		...
		log.debug("Getting coaches  for " + sport.getKey());
		Page<User> coaches = repo.listCoaches(sport.getKey().toLowerCase(), pageRequest);
		log.debug("Coaches " + coaches.getContent());
