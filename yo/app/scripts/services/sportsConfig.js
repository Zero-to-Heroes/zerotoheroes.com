	...
	function ($log, angularLoad, $parse, localStorage) {
				...
				hearthstone: {
					...
					plugins: {
						plugins: [
							...
							{name: 'joustjs', player: true, format: ['text/xml'], version: 32},
		...
		service.loadPlugin = function(plugins, pluginObj) {
			...
			if (window[pluginObj.name]) {
				...
				basket.require({ url: '/plugins/' + plugin + '/' + plugin + '.js' + version, skipCache: true, unique: 123 }).then(function () {
					plugins.push(pluginObj)
				})
				...
				if (plugin.player) {
					...
					if ((!review.mediaType && !plugin.mediaType) || review.mediaType == plugin.mediaType) {
						...
						if (window[plugin.name]) {
							...
							basket.require({ url: '/plugins/' + plugin.name + '/' + plugin.name + '.js' + version, skipCache: true, unique: 123 }).then(function () {
